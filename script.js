...
function renderTodos() {
    ...
    todos.forEach((todo, i) => {
        ...
        li.innerHTML = `<span style="flex:1" onclick="toggleTodo(${i})">${escapeHtml(todo.text)}</span><button class="del-btn" onclick="deleteTodo(${i})"><i class="fas fa-xmark"></i></button>`;
        list.appendChild(li);
    });
}
function addTodo() { const input = document.getElementById('todo-in'); const val = input.value.trim(); if (!val) return; todos.unshift({ text: val, done: false }); localStorage.setItem('todos', JSON.stringify(todos)); renderTodos(); input.value = ''; }
...
function renderLinks() {
    ...
    links.forEach((link, i) => {
        ...
        chip.innerHTML = `${escapeHtml(link.name)} <button class="link-chip-del" onclick="deleteLink(event,${i})"><i class="fas fa-xmark"></i></button>`;
